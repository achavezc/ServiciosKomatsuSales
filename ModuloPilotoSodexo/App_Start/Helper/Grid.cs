...
namespace ModuloPilotoSodexo.Helpers
{
    public static class Grid
    {
        ...
        public static String toJSONFormatForJqGrid(string rowsJson, int page, int records, int total, String llave = "")
        {
            JObject listaJqGrid = new JObject();
            listaJqGrid.Add("page", page);
            listaJqGrid.Add("records", records);
            listaJqGrid.Add("total", total);

            var jsonLinq = JArray.Parse(rowsJson);
            ...
            foreach (JObject row in srcArray.Children<JObject>())
            {
                ...
                foreach (JProperty column in row.Properties())
                {
                    ...
                    if (column.Value is JValue)
                    {
                        ...
                        jsonRows.Add(column.Name, column.Value);
                    }
                }
                JObject itemJqGrid = new JObject();
                itemJqGrid.Add("id", (mid == null ? "" : mid.ToString()));
                itemJqGrid.Add("cell", jsonRows);
                trgArray.Add(itemJqGrid);
            }


            listaJqGrid.Add("rows", trgArray);

            return GR.Scriptor.Framework.Helper.ConvertObjectToJson(listaJqGrid);
        }
