    ...
    public class ManejadorLogEventos
    {
        ...
        public void RegistrarTiempoEjecucion(RequestLogEvento request, object data = null)
        {
            try
            {
                if (!isStopped)
                    Break();
                string json = JsonConvert.SerializeObject(data, Formatting.None, new IsoDateTimeConverter() { DateTimeFormat = "dd.MM.yyyy HH:mm:ss" });// HH:mm:ss
        ...
        public void GuardarTrama2(DateTime fecha, string Tipo, string Url, string Ip, string Usuario, string Trama)
        {
            ...
            if (ConfigurationManager.AppSettings["Viatecla.Factory.Scriptor.ConnectionString"] != null && Convert.ToString(ConfigurationManager.AppSettings["LogBD"]) == "S")
            {
                cadena = ConfigurationManager.AppSettings["Viatecla.Factory.Scriptor.ConnectionString"];
                try
                {
                    using (System.Data.SqlClient.SqlConnection cn = new System.Data.SqlClient.SqlConnection(cadena))
                    {
                        System.Data.SqlClient.SqlCommand cm = new System.Data.SqlClient.SqlCommand("PA_GRSCRIPTOR_InsertLogLLamadas", cn);
                ...
                try
                {
                    ...
                    using (TransactionScope transactionScope1 = new TransactionScope(TransactionScopeOption.RequiresNew))//(TransactionScopeOption.RequiresNew, option1))
                    {
                        using (SqlConnection conn1 = new SqlConnection(cadena))
                        {
                            ...
                            System.Data.SqlClient.SqlCommand cm = new System.Data.SqlClient.SqlCommand("PA_GRSCRIPTOR_InsertLogMapa", conn1);
                            cm.CommandType = System.Data.CommandType.StoredProcedure;
